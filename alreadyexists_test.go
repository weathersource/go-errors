...
var AlreadyExistsErrorTests = []AlreadyExistsErrorTest{
	{
		...
		getCause:   nil,
		json:       []byte(`{"errorCode":409,"errorMessage":"ALREADY EXISTS. Message 1"}`),
	...
	},
	{
		...
		json:       []byte(`{"errorCode":409,"errorMessage":"ALREADY EXISTS. Message 2"}`),
		rpcCode:    codes.AlreadyExists,
...
func TestAlreadyExistsErrorAppend(t *testing.T) {

	e1       := NewAlreadyExistsError("Message 1")
	e1append := e1.Append(errors.New("foo"))
	e1alt    := NewAlreadyExistsError("Message 1", errors.New("foo"))
	assert.Equal(t, e1alt.GetCause().Error(), e1append.GetCause().Error())

	e2       :=NewAlreadyExistsError("Message 2", errors.New("foo"))
	e2append := e2.Append(errors.New("bar"))
	e2alt    := NewAlreadyExistsError("Message 2", errors.New("foo"), errors.New("bar"))
