...
var UnavailableErrorTests = []UnavailableErrorTest{
	{
		...
		getCause:   nil,
		json:       []byte(`{"errorCode":503,"errorMessage":"UNAVAILABLE. Unable to handle the request due to a temporary overloading or maintenance."}`),
...
func TestUnavailableErrorAppend(t *testing.T) {

	e1       := NewUnavailableError("Message 1")
	e1append := e1.Append(errors.New("foo"))
	e1alt    := NewUnavailableError("Message 1", errors.New("foo"))
	assert.Equal(t, e1alt.GetCause().Error(), e1append.GetCause().Error())

	e2       :=NewUnavailableError("Message 2", errors.New("foo"))
	e2append := e2.Append(errors.New("bar"))
	e2alt    := NewUnavailableError("Message 2", errors.New("foo"), errors.New("bar"))
