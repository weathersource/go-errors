...
var InvalidArgumentErrorTests = []InvalidArgumentErrorTest{
	{
		...
		getCause:   nil,
		json:       []byte(`{"errorCode":400,"errorMessage":"INVALID ARGUMENT. Message 1"}`),
	...
	},
	{
		...
		json:       []byte(`{"errorCode":400,"errorMessage":"INVALID ARGUMENT. Message 2"}`),
		rpcCode:    codes.InvalidArgument,
...
func TestInvalidArgumentErrorAppend(t *testing.T) {

	e1       := NewInvalidArgumentError("Message 1")
	e1append := e1.Append(errors.New("foo"))
	e1alt    := NewInvalidArgumentError("Message 1", errors.New("foo"))
	assert.Equal(t, e1alt.GetCause().Error(), e1append.GetCause().Error())

	e2       :=NewInvalidArgumentError("Message 2", errors.New("foo"))
	e2append := e2.Append(errors.New("bar"))
	e2alt    := NewInvalidArgumentError("Message 2", errors.New("foo"), errors.New("bar"))
