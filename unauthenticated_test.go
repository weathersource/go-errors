...
func TestUnauthenticatedErrorAppend(t *testing.T) {

	e1       := NewUnauthenticatedError("Message 1")
	e1append := e1.Append(errors.New("foo"))
	e1alt    := NewUnauthenticatedError("Message 1", errors.New("foo"))
	assert.Equal(t, e1alt.GetCause().Error(), e1append.GetCause().Error())

	e2       :=NewUnauthenticatedError("Message 2", errors.New("foo"))
	e2append := e2.Append(errors.New("bar"))
	e2alt    := NewUnauthenticatedError("Message 2", errors.New("foo"), errors.New("bar"))
