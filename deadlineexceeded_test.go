...
func TestDeadlineExceededErrorAppend(t *testing.T) {

	e1       := NewDeadlineExceededError("Message 1")
	e1append := e1.Append(errors.New("foo"))
	e1alt    := NewDeadlineExceededError("Message 1", errors.New("foo"))
	assert.Equal(t, e1alt.GetCause().Error(), e1append.GetCause().Error())

	e2       :=NewDeadlineExceededError("Message 2", errors.New("foo"))
	e2append := e2.Append(errors.New("bar"))
	e2alt    := NewDeadlineExceededError("Message 2", errors.New("foo"), errors.New("bar"))
